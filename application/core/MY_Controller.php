...
class MY_Controller extends CI_Controller {
    ...
    function __construct() {
        ...
        // set the time zone
        $timezones = date_default_timezone_set('Asia/Kolkata');
        // get current user
        ...
        $data_language = $this->languageModel->get_language_by_name($this->session->language);
        $this->session->is_rtl = NULL;
        if($data_language && $data_language->is_rtl == 1)
        { 
           $this->session->is_rtl = "RTL";
        }
        $this->session->language = $data_language ? $this->session->language : "English";

        // p($this->session->language);
        // load the core language file
        $this->lang->load('language', $this->session->language);

        // set global header data - can be merged with or overwritten in controllers
        $this->add_external_css(
            array(
                base_url("/{$this->settings->themes_folder}/admin/css/bootstrap.min.css"), 
                base_url("/{$this->settings->themes_folder}/admin/css/all.min.css"), 
        ))->add_external_js(
            array(
                base_url("/{$this->settings->themes_folder}/quizzy/js/jquery.min.js"), 
                base_url("/{$this->settings->themes_folder}/quizzy/js/jquery-ui.min.js"), 
                base_url("/{$this->settings->themes_folder}/admin/js/popper.js"), 
                base_url("/{$this->settings->themes_folder}/admin/js/tooltip.js"), 
                base_url("/{$this->settings->themes_folder}/admin/js/bootstrap.min.js"), 
                base_url("/{$this->settings->themes_folder}/admin/js/jquery.nicescroll.min.js"),
            )
        );
    ...
    function add_js_theme($js_files, $is_i18n = FALSE) {
        ...
        $url = base_url("/{$this->settings->themes_folder}/{$this->settings->theme}/js/{$js}");
//         echo '<br>'.__LINE__.'<br>';
//         print_r($url);
//         echo '<br>'.__LINE__.'<br>';
//         die('dsfsdfsdf');
